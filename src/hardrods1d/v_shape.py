...
def _get_gauss(grid1d: Grid1D, *, sigma: float) -> torch.Tensor:
    norm_fac = sigma * np.sqrt(2 * np.pi) / grid1d.L
    trans_phase = (2j * np.pi) * grid1d.iGz * 0.5  # shift to center of unit cell
    gauss_tilde = (-0.5 * (grid1d.Gmag * sigma).square() + trans_phase).exp() * norm_fac
    return gauss_tilde
